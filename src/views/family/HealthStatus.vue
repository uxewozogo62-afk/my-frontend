...
const fetchHealthData = async () => {
  try {
    ...
    // request.ts 已自动解包 res.data，所以 realtime 直接就是数据对象[cite: 2]
    const realtime = await healthApi.getRealtime(elderlyId);[cite: 1]
    
    if (realtime) {
      // --- 1. 步数处理 ---
      todaySteps.value = realtime.activitySteps || 0;

      // --- 2. 采集时间处理 (直接显示数据库原始内容) ---
      // 不使用 new Date() 转换，防止时区偏移
      lastUpdateTime.value = realtime.timestamp || '--:--:--';

      // --- 3. 所有健康指标循环赋值 (心率、血氧、血压、体温) ---
      healthMetrics.value.forEach(metric => {
        // 动态匹配 key (如 heartRate, bloodOxygen 等)
